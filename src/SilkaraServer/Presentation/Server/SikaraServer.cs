...
internal class SilkaraServerClass : BackgroundService, SilkaraServerIdenity
{
    ...
    private async Task RunServerAsync(CancellationToken ct)
    {
        try
        {
            _logger.LogInformation("SilkaraServer is running...");
            Init();
            await ListenAsync(ct);
        }
        catch when (ct.IsCancellationRequested)
        {
            _logger.LogInformation("SilkaraServer is stopping due to cancellation request.");
        }
        catch (Exception ex)
        {
    ...
    {
        _logger.LogInformation("SilkaraServer is shutting down gracefully...");
        _listener.Stop();
        ...
        _logger.LogInformation("SilkaraServer stopped cleanly(with  StopAsync method).");
    }
}
